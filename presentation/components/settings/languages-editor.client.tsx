...
export function LanguagesEditor({
  ...
  async function save() {
    ...
    setBusy(true)
    try {
      // 299-5: у элемента языки — данные APP-CONFIG (`languages: { supported, default }`), а не окружение сайта:
      // своего окружения сайта у элемента нет. Доставку набора сайту (и его пересборку) делает раздача, 299-6.
      const res = await fetch("/api/settings/app", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ languages: { supported: selected, default: def } }),
      })
