...
import { patchAtPath, mergePatches, typedValue, type Section } from "@/lib/settings/fields"
...
export function ConfigEditor({
  ...
  // Итог последней попытки — строкой под кнопкой (в элементе нет всплывающих уведомлений).
  const [status, setStatus] = useState<string | null>(null)
  const toast = { info: setStatus, error: setStatus, success: setStatus }
  ...
  async function save() {
    ...
    try {
      ...
      const res = await fetch("/api/settings/app", {
        ...
        credentials: "include",
        body: JSON.stringify(mergePatches(patches)),
      })
